"""force repair company_id columns
...
def _ensure_primary_company(bind):
    ...
    primary_company_id = bind.execute(
...
def _ensure_company_column(bind, table_name):
...
def _backfill_direct(bind, table_name, primary_company_id):
    if table_name not in _table_names(bind):
        return
    if "company_id" not in _column_names(bind, table_name):
        return
...
def _backfill_child(bind, table_name, link_column, parent_table, primary_company_id):
...
def upgrade():
    bind = op.get_bind()
    primary_company_id = _ensure_primary_company(bind)

    all_company_tables = DIRECT_COMPANY_TABLES + tuple(
        table_name for table_name, _link_column, _parent_table in CHILD_COMPANY_TABLES
    )
    for table_name in all_company_tables:
        _ensure_company_column(bind, table_name)

    for table_name in DIRECT_COMPANY_TABLES:
        _backfill_direct(bind, table_name, primary_company_id)

    for table_name, link_column, parent_table in CHILD_COMPANY_TABLES:
        _backfill_child(bind, table_name, link_column, parent_table, primary_company_id)
