...
Revision ID: 202608120005
Revises: 202608120004
...
revision = "202608120005"
down_revision = "202608120004"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    for table_name in DIRECT_COMPANY_TABLES:
        if table_name not in tables or "company_id" not in _column_names(bind, table_name):
            continue
        if table_name == "users":
            bind.execute(
                sa.text(
...
def downgrade():
    bind = op.get_bind()
    for table_name, _link_column, _parent_table in reversed(CHILD_COMPANY_TABLES):
        if table_name not in _table_names(bind) or "company_id" not in _column_names(bind, table_name):
            continue
        index_name = f"ix_{table_name}_company_id"
    ...
    for table_name in reversed(DIRECT_COMPANY_TABLES):
        if table_name not in _table_names(bind) or "company_id" not in _column_names(bind, table_name):
            continue
        index_name = f"ix_{table_name}_company_id"
        if index_name in _index_names(bind, table_name):
